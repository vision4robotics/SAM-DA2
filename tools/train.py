...
from siamban.models.mamba_discriminator import VMambaDiscriminator
import torch.nn.functional as F

logger = logging.getLogger('global')
parser = argparse.ArgumentParser(description='siamese tracking')
parser.add_argument('--cfg', type=str, default='experiments/sam-mamba-da/config.yaml',
                    help='configuration of tracking')
parser.add_argument('--model', default='sam-da-track-b', type=str,        
        help='models to train')
parser.add_argument('--discriminator_version', default='v2', type=str,        
        help='transformer or v1 or v2')
...
def train(source_loader, target_loader, model, optimizer, lr_scheduler, tb_writer, Disc, optimizer_D):
    ...
    for idx in range(cfg.TRAIN.EPOCH*num_per_epoch):#    for idx, data in enumerate(train_loader):
        ...
        # interp = nn.Upsample(size=(64, 64), mode='bilinear', align_corners=True)
...
def main():
    ...
    # load cfg
    args.cfg = '/mnt/sdc/V4R/YLL/IROS2025/BAN/experiments/sam-mamba-da/config.yaml'
    cfg.merge_from_file(args.cfg)
    cfg.DATASET.VIDEOS_PER_EPOCH = cfg.DATASET.VIDEOS_PER_EPOCH_B
    cfg.DATASET.NAT.ANNO = cfg.DATASET.NAT.ANNO_B
  

    cfg.TRAIN.LOG_DIR += str(args.model) + "-" + cfg.ALIGN.TYPE.split("_")[-1].lower()
    cfg.TRAIN.SNAPSHOT_DIR += str(args.model) + "-" + cfg.ALIGN.TYPE.split("_")[-1].lower()
    ...
    if 'trans' in args.discriminator_version:
        model_Disc = TransformerDiscriminator(channels=256, img_size=128) # 特征的通道数
    else:
        model_Disc = VMambaDiscriminator(depth=1, channels=256, num_classes=1, version=args.discriminator_version)
    model_Disc.train()
    model_Disc.cuda()
