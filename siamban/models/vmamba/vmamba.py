    ...
    def _forward(self, input: torch.Tensor):
        ...
        if self.ssm_branch:
            if self.post_norm:
                ...
                #print("ssm branch", x.shape)  # x.shape should be (batchsize, height, weight, channel)
                x = x + self.drop_path(self.op(self.norm(x)))
        if self.mlp_branch:
            if self.post_norm:
                x = x + self.drop_path(self.norm2(self.mlp(x))) # FFN
            else:
                #print("mlp branch", x.shape)
...
'''
the basic vmamba tracking neck with little modification
'''
class VMambaNeck(nn.Module):
    def __init__(
        self, 
        patch_size=4, 
        in_chans=3, 
        ...
        **kwargs,
        ...
        self.pos_embed_1 = self._pos_embed(dims[0], 7, 7) if posembed else None
        self.pos_embed_2 = self._pos_embed(dims[0], 31, 31) if posembed else None

        # we don't need patch embedding
        '''
        _make_patch_embed = dict(
            v1=self._make_patch_embed, 
            v2=self._make_patch_embed_v2,
        ...
        self.layers = nn.ModuleList()
        for i_layer in range(self.num_layers):
            # we don't need downsampling, replaced with nn.Identity()
            '''
            downsample = _make_downsample(
                self.dims[i_layer], 
                self.dims[i_layer + 1], 
    ...
    @staticmethod
    def _pos_embed(embed_dims, patch_height, patch_width):
        pos_embed = nn.Parameter(torch.zeros(1, embed_dims, patch_height, patch_width))
        trunc_normal_(pos_embed, std=0.02)
        return pos_embed
    ...
    # we don't need patch embedding or downsampling
    '''
    @staticmethod
    def _make_patch_embed(in_chans=3, embed_dim=96, patch_size=4, patch_norm=True, norm_layer=nn.LayerNorm, channel_first=False):
        # if channel first, then Norm and Output are both channel_first
    ...
    def forward(self, x: torch.Tensor):
        # x = self.patch_embed(x)
        if self.pos_embed_1 is not None and self.pos_embed_2 is not None:
            if x.shape[1]==7:
                pos_embed = self.pos_embed_1.permute(0, 2, 3, 1) if not self.channel_first else self.pos_embed
            elif x.shape[1]==31:
                pos_embed = self.pos_embed_2.permute(0, 2, 3, 1) if not self.channel_first else self.pos_embed
            x = x + pos_embed
        ...
        **kwargs,
        ...
        if self.soft_thresholding:
            ...
            self.fc = nn.Sequential(
                nn.Linear(hidden_dim, int(hidden_dim * se_ratio)),
                nn.BatchNorm1d(int(hidden_dim * se_ratio)),
                nn.ReLU(inplace=True),
                nn.Linear(int(hidden_dim * se_ratio), hidden_dim),
                nn.Sigmoid(),
            )

    def _soft_thresholding(self, x):
        # x.shape should be (batchsize, height, weight, channel)
        #print("input to soft thresholding", x.shape)
        x = x.permute(0, 3, 1, 2)  # permuted to (batchsize, channel, height, weight)
        #print("permuted to ", x.shape)
        x1 = self.gap(x.abs())  # (batchsize, channel, height, weight) --> (batchsize, channel, 1, 1)
        #print("gaped to", x1.shape)
        x2 = self.fc(torch.flatten(x1, 1)).unsqueeze(-1).unsqueeze(-1)  # (batchsize, channel)-->(batchsize, channel,1,1)
        #print("channel attention", x2.shape)
        threshold = torch.mul(x1, x2)  # (batchsize, channel, 1, 1)
        #print('threshold[0,:,0,0]', threshold[0,:,0,0])
        #print("calculate threshold", threshold.shape)
        sign = torch.sign(x)
        value = x.abs() - threshold.abs().expand_as(x)
        zeros = torch.zeros_like(value)
        value = torch.max(value, zeros)
        #print('value[0,0,:,:]', value[0,0,:,:])
        return torch.mul(sign, value).permute(0, 2, 3, 1)

    def _forward(self, input: torch.Tensor):
        x = input  # x.shape should be (batchsize, height, weight, channel)
        if self.ssm_branch and self.soft_thresholding:
            if self.post_norm:
                x = x + self.drop_path(self._soft_thresholding(self.norm(self.op(x))))
            else:
                x = x + self.drop_path(self._soft_thresholding(self.op(self.norm(x))))
        elif self.ssm_branch:
            if self.post_norm:
                x = x + self.drop_path(self.norm(self.op(x)))
            else:
                x = x + self.drop_path(self.op(self.norm(x)))
...
if __name__ == '__main__':    
    model = VMambaNeckV2(depths=[1], dims=[256], posembed=False)
    numparams = sum(p.numel() for p in model.parameters())
    print("1层param", numparams)  # 1357824

    model = VMambaNeckV2(depths=[1]*2, dims=[256]*2, posembed=False)
    numparams = sum(p.numel() for p in model.parameters())
    print("2层param", numparams)  # 2715648
